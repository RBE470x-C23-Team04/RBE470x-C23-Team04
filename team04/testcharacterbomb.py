...
class TestCharacterBomb(CharacterEntity):
    ...
    def heuristic(self, wrld):
        ...
        dist = TestCharacterBomb.euclidean_distance(m[0].x,m[0].y,c[0].x,c[0].y)
    ...
    def a_star(self, wrld, start, goal):
        ...
        while not frontier.empty():
            ...
            for next in TestCharacterBomb.neighbors_of_8(wrld, current[0], current[1]):
                new_cost = cost_so_far[current] + TestCharacterBomb.euclidean_distance(current[0],current[1],next[0],next[1]) #+ TestCharacter2.mSpace(next[0],next[1])
    ...
    def do(self, wrld):
        ...
        if state == "findCharacters":
        ...
        elif state == "findMonsters":
            m = next(iter(wrld.monsters.values())) #use wrld method. sense world or real?
            monster_pos = self.findMonsterPos(wrld, "stupid") #use our own method. sense world or real?
            ...
            print("A* Path: " + str(pose_list))
        elif state == "placeBomb":
            """ From path, find spaces that make sense to place bombs on, then place a
            bomb on that space when character gets to it.
            """
            
            pass
        elif state == "avoidBomb":
            pass
        elif state == "movement":
        ...
        elif state == "avoidMonster":
            pass
        else:
            print("Place holder for error or something else.")
        ...
        # # dx, dy = 0, 0 # starting pos --not always!
        # dx, dy = character_pos[0], character_pos[1]
        # start = (dx, dy)
        # goal = self.findGoal(wrld)
        # print("Goal " + str(goal)) 
